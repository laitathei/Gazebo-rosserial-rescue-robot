...
init_orientation_anticlockwise = 1.0
moved_distance = 0.0
...
if __name__ == "__main__":
    ...
    while check == False:
        ...
        print "1. moving forward (press W four times immediately) (around 30s)"
        print "2. moving backward (press S four times immediately) (around 30s)"
        ...
        if choice == 1:
            for i in range(buffertime_forward_mode):
                ...
                # pose
                data_odom.pose.position.x = current_x_position                  # unchange
                data_odom.pose.position.y = (moved_distance+saved_y_position)/100.0     # change
                data_odom.pose.position.z = 0.05               # unchange
            ...
            for i in range(buffertime_backward_mode):
            ...
            for i in range(buffertime_clockwise_mode):
                ...
                if moved_angular_distance < 41.0:  # rotate to left < 90 degree
                    moved_radian = init_orientation_anticlockwise+moved_radian
            ...
            for i in range(buffertime_anticlockwise_mode):
        ...
        if choice == 5:
            for i in range(buffertime_left_mode):
        ...
        if choice == 6:
            for i in range(buffertime_right_mode):
