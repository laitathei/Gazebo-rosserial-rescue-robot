...
class OdomEncoder:
    def __init__(self):
        ...
        self.facing_backward = 1
        self.facing_left = 1
        self.facing_right = 1
        self.facing_quadrant1 = 1
        self.facing_quadrant2 = 1
        self.facing_quadrant3 = 1
        self.facing_quadrant4 = 1
        self.over_half = 1
    ...
    def update(self):
        ...
        if (self.current_z_orientation < 0.06)&(self.current_z_orientation > -0.06):
            self.facing_forward = 1
            self.facing_backward = 0
            self.facing_left = 0
            self.facing_right = 0
            self.facing_quadrant1 = 0
            self.facing_quadrant2 = 0
            self.facing_quadrant3 = 0
            self.facing_quadrant4 = 0
            self.positive_x_origin = False
            self.negative_x_origin = False
            ...
            self.over_half = 1
        # facing backward
        if ((self.current_z_orientation > 0.99)&(self.current_z_orientation <1))or((self.current_z_orientation < -0.994)&(self.current_z_orientation > -1)):
            self.facing_forward = 0
            self.facing_backward = 1
            self.facing_left = 0
            self.facing_right = 0
            self.facing_quadrant1 = 0
            ...
            self.over_half = 0
        # facing right
        if (self.current_z_orientation < -0.687) & (self.current_z_orientation > -0.73):
            self.facing_forward = 0
            self.facing_backward = 0
            self.facing_left = 0
            self.facing_right = 1
        ...
        if (self.current_z_orientation > 0.687) & (self.current_z_orientation < 0.73):
            ...
            self.negative_y_origin = False
        # facing quadrant1
        if (self.current_z_orientation < -0.01) & (self.current_z_orientation > -0.687):
            self.facing_forward = 0
            self.facing_backward = 0
            self.facing_left = 0
            self.facing_right = 0
        ...
        # facing quadrant3
        if (self.current_z_orientation > 0.73) & (self.current_z_orientation < 0.99):
            self.facing_forward = 0
            self.facing_backward = 0
            self.facing_left = 0
            self.facing_right = 0
    ...
    def calculation(self):
        ...
        # turn left
            ...
            if (self.over_half == 0)&(self.current_z_orientation < 0.06)&(self.current_z_orientation > -0.707):
                self.moved_distance = self.moved_distance*3.4                                                   # previous 5.4
            elif (self.over_half == 0)&(self.current_z_orientation < -0.707)&(self.current_z_orientation > -0.757):
                self.moved_distance = self.moved_distance*4.413                                             # previous 6.413
            elif (self.over_half == 0)&(self.current_z_orientation < -0.757)&(self.current_z_orientation > -0.807):
                self.moved_distance = self.moved_distance*4.413                                             # previous 7.413
                ...
                self.moved_distance = self.moved_distance*55.0                            # previous 85.0
            ...
            if (self.over_half == 1)&(self.current_z_orientation > 0.06)&(self.current_z_orientation < 0.707):
                self.moved_distance = self.moved_distance*2.4                                             # previous 5.4
                ...
                self.moved_distance = self.moved_distance*5.413                                             # previous 7.413
            elif (self.over_half == 1)&(self.current_z_orientation > 0.907)&(self.current_z_orientation < 0.927):
                self.moved_distance = self.moved_distance*5.413                                             # previous 10.413
            elif (self.over_half == 1)&(self.current_z_orientation > 0.927)&(self.current_z_orientation < 0.957):
                self.moved_distance = self.moved_distance*7.413                                             # previous 11.413
            ...
            if (self.over_half == 1):
                print (self.moved_distance)
                self.moved_angular_distance = (-self.moved_distance)*(-self.moved_distance)
                print "The moved distance (cm) is : "+ str(-self.moved_distance)
                print "The rotated distance (turn left) (cm) is : "+ str(self.moved_angular_distance)
                self.moved_radian = self.moved_angular_distance
                self.abs_moved_radian = self.moved_radian
                print ""
            # turn left parameter tuning facing backward (from 180 degree to 360 degree)
            if (self.over_half == 0):
                print (self.moved_distance)
                self.moved_angular_distance = (-self.moved_distance)*(-self.moved_distance)
                print "The moved distance (cm) is : "+ str(-self.moved_distance)
            ...
            # turn left parameter tuning facing forward (almost over half)
        ...
        # turn right
            ...
            if (self.over_half == 1):
                ...
                self.moved_angular_distance = self.moved_distance*self.moved_distance
                print "The moved distance (cm) is : "+ str(self.moved_distance)
                print "The rotated distance (turn right) (cm) is : "+ str(self.moved_angular_distance)
                self.moved_radian = self.moved_angular_distance
                self.abs_moved_radian = self.moved_radian
                print ""
            # from 180 to 360 degree
            if (self.over_half == 0):
                print (self.moved_distance)
                self.moved_angular_distance = self.moved_distance*self.moved_distance
                print "The moved distance (cm) is : "+ str(self.moved_distance)
            ...
            # turn right parameter tuning when facing forward (almost over half)
            if (self.over_half == 1)&(self.current_z_orientation < -0.999)&(self.current_z_orientation > -0.999999999):
                    self.moved_radian = 100.0                            # previous 1000000000000000000000
                    self.saved_moved_radian = 100.0
                    self.data_odom_right = ModelState()
    ...
    def define_quadrant(self):
        ...
        if (self.current_x_position > (self.boundary_parameter-0.0875)) & (self.current_y_position > (self.boundary_parameter-0.0875)):
            self.quadrant = 1
            if ((self.current_z_orientation < -0.01) & (self.current_z_orientation > -0.687))or((self.current_z_orientation > 0.01) & (self.current_z_orientation < 0.687))or((self.current_z_orientation > 0.73) & (self.current_z_orientation < 0.99))or((self.current_z_orientation < -0.73) & (self.current_z_orientation > -0.99)):
                self.move_crossline = True
            elif ((self.current_z_orientation < 0.01) & (self.current_z_orientation > -0.01))or((self.current_z_orientation < -0.99) & (self.current_z_orientation > -1.0))or((self.current_z_orientation < -0.687) & (self.current_z_orientation > -0.73))or((self.current_z_orientation > 0.687) & (self.current_z_orientation < 0.73)):
                self.move_crossline = False
        # located in quadrant 2 x<-0.0125, y>0.0125
        if (self.current_x_position < self.negative_boundary_parameter+0.0875) & (self.current_y_position > self.boundary_parameter-0.0875):
            ...
            if ((self.current_z_orientation < -0.01) & (self.current_z_orientation > -0.687))or((self.current_z_orientation > 0.01) & (self.current_z_orientation < 0.687))or((self.current_z_orientation > 0.73) & (self.current_z_orientation < 0.99))or((self.current_z_orientation < -0.73) & (self.current_z_orientation > -0.99)):
                ...
                self.move_crossline = False
        # located in quadrant 3 x<-0.0125, y<-0.0125
        if (self.current_x_position < self.negative_boundary_parameter+0.0875) & (self.current_y_position < self.negative_boundary_parameter+0.0875):
            ...
            if ((self.current_z_orientation < -0.01) & (self.current_z_orientation > -0.687))or((self.current_z_orientation > 0.01) & (self.current_z_orientation < 0.687))or((self.current_z_orientation > 0.73) & (self.current_z_orientation < 0.99))or((self.current_z_orientation < -0.73) & (self.current_z_orientation > -0.99)):
                ...
                self.move_crossline = False
        # located in quadrant 4 x>0.0125, y<-0.0125
    ...
    def turn_right_function(self):
        ...
        if (self.over_half == 1)&(self.current_z_orientation > -0.999):
            ...
            print ""
        # turn left 180 to 360 degree
        if (self.over_half == 0)&(self.current_z_orientation < 0.999):
            self.data_odom_right = ModelState()
            self.data_odom_right.model_name = 'robot'
    ...
    def turn_left_function(self):
        # turn left 0 to 180 degree
        if (self.over_half == 1)&(self.current_z_orientation < 0.9999999):
            self.data_odom_right = ModelState()
            self.data_odom_right.model_name = 'robot'
            ...
            print ""
        # turn left 180 to 360 degree
        if (self.over_half == 0)&(self.current_z_orientation > -0.9999999):
            self.data_odom_right = ModelState()
            self.data_odom_right.model_name = 'robot'

            self.moved_radian = self.saved_moved_radian+self.abs_moved_radian
            self.saved_moved_radian = -abs(self.moved_radian)
    ...
    def located_in_origin(self):
        ...
        if ((self.move_crossline == False)&(self.current_z_orientation < 0.01) & (self.current_z_orientation > -0.01) & (self.quadrant == 0) & (self.leave_origin == False)):
                ...
                if abs(self.current_y_position) > self.boundary_parameter:
        ...
        if ((self.move_crossline == False)&(self.current_z_orientation < -0.687) & (self.current_z_orientation > -0.73) & (self.quadrant == 0) & (self.leave_origin == False)):
        ...
        # facing quadrant 1
        if ((self.move_crossline == True)&(self.current_z_orientation < -0.01) & (self.current_z_orientation > -0.687) & ((self.quadrant == 0)or(self.quadrant == 1)or(self.quadrant == 3)) & (self.leave_origin == False)):
                self.distance_from_x_origin = float(self.moved_distance+self.saved_x_position)
                self.distance_from_y_origin = float(self.moved_distance+self.saved_y_position)
                ...
                print ""
                # move to quadrant 1 and leave the origin box x>0.1, y>0.1
                if (self.facing_quadrant1 == 1)&(self.current_x_position > (self.boundary_parameter-0.09)) & (self.current_y_position > (self.boundary_parameter-0.09)):
                    self.leave_origin = True
                    self.positive_quadrant1_origin = True
                    self.negative_quadrant1_origin = False
                    self.leave_origin_orientation_z = self.current_z_orientation
                # move to quadrant 3 and leave the origin box x>-0.1, y>-0.1
                if (self.facing_quadrant1 == 1)&(self.current_x_position > (self.negative_boundary_parameter+0.09)) & (self.current_y_position > (self.negative_boundary_parameter+0.09)):
                    self.leave_origin = True
                    self.positive_quadrant1_origin = False
                    self.negative_quadrant1_origin = True
                    self.leave_origin_orientation_z = self.current_z_orientation
        ...
        # facing quadrant 2
        if ((self.move_crossline == True)&(self.current_z_orientation > 0.01) & (self.current_z_orientation < 0.687) & ((self.quadrant == 0)or(self.quadrant == 2)or(self.quadrant == 4)) & (self.leave_origin == False)):
                self.distance_from_x_origin = float(-self.moved_distance+self.saved_x_position)
                self.distance_from_y_origin = float(self.moved_distance+self.saved_y_position)
                ...
                print ""
                # move to quadrant 2 and leave the origin box x<-0.1, y>0.1
                if (self.facing_quadrant2 == 1)&(self.facing_quadrant2 == 1)&(self.current_x_position < (self.negative_boundary_parameter+0.09)) & (self.current_y_position > (self.boundary_parameter-0.09)):
                    self.leave_origin = True
                    self.positive_quadrant2_origin = True
                    self.negative_quadrant2_origin = False
                    self.leave_origin_orientation_z = self.current_z_orientation
                # move to quadrant 4 and leave the origin box x>0.1, y<-0.1
                if (self.facing_quadrant2 == 1)&(self.facing_quadrant2 == 1)&(self.current_x_position > (self.boundary_parameter-0.09)) & (self.current_y_position < (self.negative_boundary_parameter+0.09)):
                    self.leave_origin = True
                    self.positive_quadrant2_origin = False
                    self.negative_quadrant2_origin = True
                    self.leave_origin_orientation_z = self.current_z_orientation
        ...
        # facing quadrant 3
        if ((self.move_crossline == True)&(self.current_z_orientation > 0.73) & (self.current_z_orientation < 0.99) & ((self.quadrant == 0)or(self.quadrant == 1)or(self.quadrant == 3)) & (self.leave_origin == False)):
                self.distance_from_x_origin = float(-self.moved_distance+self.saved_x_position)
                self.distance_from_y_origin = float(-self.moved_distance+self.saved_y_position)
                ...
                print ""
                # move to quadrant 3 and leave the origin box x<-0.1, y<-0.1
                if (self.facing_quadrant3 == 1)&(self.current_x_position < (self.negative_boundary_parameter+0.09)) & (self.current_y_position < (self.negative_boundary_parameter+0.09)):
                    self.leave_origin = True
                    self.positive_quadrant3_origin = True
                    self.negative_quadrant3_origin = False
                    self.leave_origin_orientation_z = self.current_z_orientation
                # move to quadrant 1 and leave the origin box x>0.1, y>0.1
                if (self.facing_quadrant3 == 1)&(self.current_x_position > (self.boundary_parameter-0.09)) & (self.current_y_position > (self.boundary_parameter-0.09)):
                    self.leave_origin = True
                    self.positive_quadrant3_origin = False
                    self.negative_quadrant3_origin = True
                    self.leave_origin_orientation_z = self.current_z_orientation
        ...
        # facing quadrant 4
        if ((self.move_crossline == True)&(self.current_z_orientation < -0.73) & (self.current_z_orientation > -0.99) & ((self.quadrant == 0)or(self.quadrant == 2)or(self.quadrant == 4)) & (self.leave_origin == False)):
                self.distance_from_x_origin = float(self.moved_distance+self.saved_x_position)
                self.distance_from_y_origin = float(-self.moved_distance+self.saved_y_position)
                ...
                print ""
                # move to quadrant 4 and leave the origin box x>0.1, y<-0.1
                if (self.facing_quadrant4 == 1)&(self.current_x_position > (self.boundary_parameter-0.09)) & (self.current_y_position < (self.negative_boundary_parameter+0.09)):
                    self.leave_origin = True
                    self.positive_quadrant4_origin = True
                    self.negative_quadrant4_origin = False
                    self.leave_origin_orientation_z = self.current_z_orientation
                # move to quadrant 2 and leave the origin box x<-0.1, y>0.1
                if (self.facing_quadrant4 == 1)&(self.current_x_position < (self.negative_boundary_parameter+0.09)) & (self.current_y_position > (self.boundary_parameter-0.09)):
                    self.leave_origin = True
                    self.positive_quadrant4_origin = False
                    self.negative_quadrant4_origin = True
                    self.leave_origin_orientation_z = self.current_z_orientation
    ...
    def moving_XY_axis_publish(self):
        ...
        if (self.facing_forward == 1)&(self.positive_y_origin == True) :
            self.distance_from_y_origin = float(self.moved_distance+self.saved_y_position)
        ...
        elif (self.facing_backward == 1)&(self.negative_y_origin == True) :
            if (self.move_forward == True)&(self.move_backward == False):
                self.distance_from_y_origin = float(-self.moved_distance+self.saved_y_position)
            elif (self.move_backward == True)&(self.move_forward == False):
                self.distance_from_y_origin = float(-self.moved_distance+self.saved_y_position)
        ...
        elif (self.facing_right == 1)&(self.positive_x_origin == True) :
            self.distance_from_x_origin = float(self.moved_distance+self.saved_x_position)
        ...
        elif (self.facing_left == 1)&(self.negative_x_origin == True) :
            if (self.move_forward == True)&(self.move_backward == False):
                self.distance_from_x_origin = float(-self.moved_distance+self.saved_x_position)
            elif (self.move_backward == True)&(self.move_forward == False):
                self.distance_from_x_origin = float(-self.moved_distance+self.saved_x_position)
    ...
    def moving_quadrant_publish(self):
        ...
        # facing quadrant 1 and move to quadrant 1 and move to quadrant 3
        if (self.facing_quadrant1 == 1)&(self.positive_quadrant1_origin == True)&(self.negative_quadrant3_origin == True)&(self.negative_quadrant1_origin == True)&(self.positive_quadrant3_origin == True)&(self.positive_quadrant2_origin == False)&(self.negative_quadrant2_origin == False)&(self.positive_quadrant4_origin == False)&(self.negative_quadrant4_origin == False)  :
            self.distance_from_x_origin = float(self.moved_distance+self.saved_x_position)
            self.distance_from_y_origin = float(self.moved_distance+self.saved_y_position)
            ...
            self.data_odom_quadrant.pose.orientation.z = self.current_z_orientation            # unchange
        ...
        # facing quadrant 2 and move to quadrant 2 and move to quadrant 4
        if (self.facing_quadrant2 == 1)&(self.positive_quadrant2_origin == True)&(self.negative_quadrant4_origin == True)&(self.negative_quadrant2_origin == True)&(self.positive_quadrant4_origin == True)&(self.positive_quadrant1_origin == False)&(self.negative_quadrant1_origin == False)&(self.positive_quadrant3_origin == False)&(self.negative_quadrant3_origin == False) :
            self.distance_from_x_origin = float(-self.moved_distance+self.saved_x_position)
            self.distance_from_y_origin = float(self.moved_distance+self.saved_y_position)
        ...
        # facing quadrant 3 and move to quadrant 3 and move to quadrant 1
        if (self.facing_quadrant3 == 1)&(self.positive_quadrant3_origin == True)&(self.negative_quadrant1_origin == True)&(self.negative_quadrant3_origin == True)&(self.positive_quadrant1_origin == True)&(self.positive_quadrant2_origin == False)&(self.negative_quadrant2_origin == False)&(self.positive_quadrant4_origin == False)&(self.negative_quadrant4_origin == False)   :
            self.distance_from_x_origin = float(-self.moved_distance+self.saved_x_position)
            self.distance_from_y_origin = float(-self.moved_distance+self.saved_y_position)
            ...
            self.data_odom_quadrant.pose.orientation.x = self.current_x_orientation               # unchange
            self.data_odom_quadrant.pose.orientation.y = self.current_y_orientation               # unchange
            self.data_odom_quadrant.pose.orientation.z = self.current_z_orientation+1            # unchange
            self.data_odom_quadrant.pose.orientation.w = 1.0            # unchange
        ...
        # facing quadrant 4 and move to quadrant 4 and move to quadrant 2
        if (self.facing_quadrant4 == 1)&(self.positive_quadrant4_origin == True)&(self.negative_quadrant2_origin == True)&(self.negative_quadrant4_origin == True)&(self.positive_quadrant2_origin == True)&(self.positive_quadrant1_origin == False)&(self.negative_quadrant1_origin == False)&(self.positive_quadrant3_origin == False)&(self.negative_quadrant3_origin == False)  :
            self.distance_from_x_origin = float(self.moved_distance+self.saved_x_position)
            self.distance_from_y_origin = float(-self.moved_distance+self.saved_y_position)
...
if __name__ == "__main__":
        try:
            ...
            while not rospy.is_shutdown():
                ...
                print (odom.facing_backward)
                print "facing_left :"
                print (odom.facing_left)
                print "facing_right :"
                print (odom.facing_right)
                print "facing_quadrant1 :"
                print (odom.facing_quadrant1)
                print "facing_quadrant2 :"
                print (odom.facing_quadrant2)
                print "facing_quadrant3 :"
                print (odom.facing_quadrant3)
                print "facing_quadrant4 :"
                print (odom.facing_quadrant4)
                print "move_crossline :"
                print (odom.move_crossline)
                print "leave_origin_orientation_z :"
                print (odom.leave_origin_orientation_z)
                print "----------------------------"
                ...
                # Move wherever when it not leave the origin yet
                if ((odom.facing_forward == 1)or(odom.facing_backward == 1)or(odom.facing_left == 1)or(odom.facing_right == 1)or(odom.facing_quadrant1 == 1)or(odom.facing_quadrant2 == 1)or(odom.facing_quadrant3 == 1)or(odom.facing_quadrant4 == 1))&((odom.quadrant == 1)or(odom.quadrant == 2)or(odom.quadrant == 3)or(odom.quadrant == 4)or(odom.quadrant == 0))& (odom.leave_origin == False)&(odom.turn_left == False)&(odom.turn_right == False)&(((odom.current_left>odom.saved_left)&(odom.current_right>odom.saved_right))or((odom.saved_left>odom.current_left)&(odom.saved_right>odom.current_right))):
                    odom.located_in_origin()
                # Move wherever when it leaves the origin yet
                if ((odom.facing_forward == 1)or(odom.facing_backward == 1)or(odom.facing_left == 1)or(odom.facing_right == 1)or(odom.facing_quadrant1 == 1)or(odom.facing_quadrant2 == 1)or(odom.facing_quadrant3 == 1)or(odom.facing_quadrant4 == 1))&((odom.quadrant == 1)or(odom.quadrant == 2)or(odom.quadrant == 3)or(odom.quadrant == 4)or(odom.quadrant == 0))& (odom.leave_origin == True)&(odom.turn_left == False)&(odom.turn_right == False)&(((odom.current_left>odom.saved_left)&(odom.current_right>odom.saved_right))or((odom.saved_left>odom.current_left)&(odom.saved_right>odom.current_right))):
