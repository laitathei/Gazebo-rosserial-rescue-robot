...
ros::Publisher LeftEncoder_value("LeftEncoder_value", &str_msg);     //set rostopic name 
ros::Publisher RightEncoder_value("RightEncoder_value", &str_msgR);
...
void countL() {
  ...
  if ((forward_L == true)&&(backward_L == false))
  {
  pulses_L++;
  }
  else if ((forward_L == false)&&(backward_L == true))
  {
  pulses_L--;
  }
}
...
void countR() {
  if ((forward_R == true)&&(backward_R == false))
  {
  pulses_R++;
  }
  if ((forward_R == false)&&(backward_R == true))
  {
  pulses_R--;
  }
}
...
void setup(){
    Motors_init();
    nh.initNode();
    nh.advertise(LeftEncoder_value);
    nh.advertise(RightEncoder_value);
    nh.subscribe(sub);
...
void loop(){
 /*   Serial.write("speed_lin");
    ...
    LeftEncoder_value.publish( &str_msg );   //publish the pulses_L value 
    RightEncoder_value.publish(&str_msgR);
    delay(100);
